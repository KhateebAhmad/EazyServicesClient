...
function fetchServices(){
	$.ajax({
		...
		type: "GET",
		url:'http://localhost:8084/tapserve/services',
...
function searchSP(){
	...
	$.ajax({
		contentType: 'application/JSON',
		type: "GET",
		url: 'http://localhost:8084/tapserve/findServiceProviders/'+id+'/'+$('#regionIdentifier').val()+'/'+$('#servicesDropdown').val(),
		success: function(result){
			$('#dataDiv').empty();
			searchResult = result;
...
function appointmentRequest(){
	...
	$.ajax({
		contentType: 'application/JSON',
		type: "POST",
		url: 'http://localhost:8084/tapserve/'+user.id+'/bookAppointment/'+serviceProvider.id+'/'+service.id,
		data: jsonString,
		success: function(result){
			$('#appointmentFields').hide();
			if(result.id != ""){
...
function showAppointmentListForUser(){
	...
	$.ajax({
		contentType: 'application/JSON',
		type: "GET",
		url: 'http://localhost:8084/tapserve/'+id+'/appointments',
		success: function(result){
...
function showAppointmentDetail(count){
	...
	$.ajax({
		contentType: 'application/JSON',
		type: "GET",
		url: 'http://localhost:8084/tapserve/'+userId+'/appointments/'+appointmentId,
		success: function(result){
			...
			if(result.status == "0"){
				...
				$('#appointmentDetailsSpace').append('<div id="appointmentDetailsPane" class="w3-panel w3-card-8 w3-theme-d1"> <p> Appointment Name : '+result.name+'&nbsp;&nbsp; Booking Date : '+(new Date(result.bookingDate)).toLocaleString()+' &nbsp; Appointment Date : '+(new Date(result.appointmentDate)).toLocaleString()+' </p> <p>Description : '+result.description+' &nbsp; Address : '+result.address+' &nbsp; Contact No. : '+result.contactNumber+'</p> <p>Service Provider Name : '+result.serviceProvider.name+' &nbsp;&nbsp; Service Name : '+result.service.name+'</p></div> <div id="reviewPane" class="w3-panel w3-card-8 w3-theme-d1"><div class="group"><label for="user" class="label">Review</label> <textarea id="review" ></textarea></div><div class="group"><label for="user" class="label">Rating</label><select name="rating" class="input" id="rating"><option value="0" style="color: black;">--Select--</option><option style="color: black;" value="1">1</option><option style="color: black;" value="2">2</option><option style="color: black;" value="3">3</option><option style="color: black;" value="4">4</option><option style="color: black;" value="5">5</option></select></div><div class="group"><input type="button" class="button" value="Post Review" onclick="postReview('+count+');"></div></div>');
			}
			}
	});
}
...
function makePayment(count){
	...
	$.ajax({
		contentType: 'application/JSON',
		type: "POST",
		url: 'http://localhost:8084/tapserve/'+userId+'/payment/'+appointmentId,
		data: jsonString,
		success: function(result){
			$('#paymentPane').empty();
			$('#paymentPane').hide();
...
function postReview(count){
	...
	$.ajax({
		contentType: 'application/JSON',
		type: "POST",
		url: 'http://localhost:8084/tapserve/'+userId+'/postReview/'+serviceProviderId,
		data: jsonString,
		success: function(result){
			$('#reviewPane').empty();
			$('#reviewPane').hide();
