...
function signUp(){
	...
	$.ajax({
		...
		type: "POST",
		url: 'http://localhost:8084/tapserve/register',
		data: jsonString,
...
function login(){
	...
	$.ajax({
		contentType: 'application/JSON',
		type: "POST",
		url: 'http://localhost:8084/tapserve/login',
		data: jsonString,
		success: function(result){
			var usertype = result.role.name;
			$('#generatedId').val(result.id);
			if(usertype == "USER"){
				$(document).attr("title", "EazyServe - User");
				...
				$(document).attr("title", "EazyServe - Service Provider");
